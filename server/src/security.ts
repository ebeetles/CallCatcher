...
// ---------- signed state (OAuth connect links) ----------
// The Google consent link is generated by the operator and opened by the CLIENT
// in a browser with no dashboard session, so the callback can't rely on auth.
// Instead we bind the businessId into an HMAC-signed, expiring `state` param.
// Format: base64url(payloadJson) + "." + base64url(hmac). Payload = {b, exp}.

function stateKey(): string {
  // Any stable server secret works; the token only proves WE minted it.
  return config.secretsKey || config.dashboardToken || config.supabaseJwtSecret || "callcatcher-state";
}
...
/** Sign an OAuth `state` binding a businessId, valid for ttlMs (default 7 days). */
export function signState(businessId: string, ttlMs = 7 * 24 * 60 * 60_000): string {
  const payload = b64url(Buffer.from(JSON.stringify({ b: businessId, exp: Date.now() + ttlMs })));
  const sig = b64url(createHmac("sha256", stateKey()).update(payload).digest());
  return `${payload}.${sig}`;
}

/** Verify a signed state; returns the businessId or undefined if bad/expired. */
export function verifyState(state: string | undefined): string | undefined {
  if (!state || !state.includes(".")) return undefined;
  const [payload, sig] = state.split(".");
  const expected = b64url(createHmac("sha256", stateKey()).update(payload).digest());
  if (!safeEqual(expected, sig)) return undefined;
  try {
    const { b, exp } = JSON.parse(unb64url(payload).toString("utf8"));
    if (typeof b !== "string" || typeof exp !== "number" || exp < Date.now()) return undefined;
    return b;
  } catch {
    return undefined;
  }
